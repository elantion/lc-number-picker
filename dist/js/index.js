...
var NumberPicker = (function () {
    function NumberPicker(args) {
        this.$pickers = [];
        this._indicator = [];
        this._sizeRatio = 30;
        this._numberHeight = 180;
        this._fontSize = 5;
        this._outArray = [];
        args = args || {};
    ...
    NumberPicker.prototype.insert = function (dom) {
    ...
    NumberPicker.prototype.movePicker = function (i, from, to, duration, timingFunction) {
        var h = 180;
        var t = to % (10 * h);
        var p = this.$pickers[i];
        duration = duration || 0;
        timingFunction = timingFunction || TWEEN.Easing.Linear.None;
        var tween = new TWEEN.Tween({ y: -9 * h + from })
            .to({ y: -9 * h + t }, duration)
            .onUpdate(function () {
    ...
    NumberPicker.prototype.genPicker = function (index) {
    ...
    NumberPicker.prototype.touchstart = function (index, event) {
        this._sx = event.touches[0].clientY;
        this._touchIndex = index;
    };
    NumberPicker.prototype.touchmove = function (event) {
        var d = event.touches[0].clientY - this._sx;
        var from = this._indicator[this._touchIndex];
        var to = this._indicator[this._touchIndex] + d;
        this.movePicker(this._touchIndex, from, to);
        this._sx = this._sx + d;
        this._indicator[this._touchIndex] = to % (10 * this._numberHeight);
    };
    NumberPicker.prototype.touchend = function () {
        this.numberSelect();
    };
    NumberPicker.prototype.numberSelect = function () {
        var t = this._indicator[this._touchIndex];
        var to = t - t % this._numberHeight;
        this.movePicker(this._touchIndex, t, to, 300, TWEEN.Easing.Cubic.Out);
        this._indicator[this._touchIndex] = to % (10 * this._numberHeight);
        var num = +(this._indicator[this._touchIndex] / this._numberHeight).toString().split('.')[0];
        if (num === 0) {
            num = 0;
        }
        else if (num < 0) {
    ...
    NumberPicker.prototype.addEventListen = function (event, callback) {
        if (event === 'change') {
